  ...
  public  class XMLProcess
    {
        ...
        /// <param name="node">要查询的节点</param>
        /// <returns></returns>
        public static XmlNodeList ReadAllChild(string path, string node)
        {
        ...
        /// <param name="strList">strList是一个二维数组，它的值是由xml属性名和值组成的</param>
        /// <returns></returns>
        public static void InsertHash(string path, string node, string element, Hashtable strList)
        {
            try
            {
                XmlDocument doc = new XmlDocument();
                ...
                {
                    strAttribute =de.Key.ToString();
                    strValue = de.Value.ToString();
        ...
        /// <returns<〈/returns>
        public static DataView GetDataViewByXml(string XMLFile1, string strWhere, string strSort)
        {
            try
            {
                string filename = AppDomain.CurrentDomain.BaseDirectory.ToString() + XMLFile1;
