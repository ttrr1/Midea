...
using System.Web;
using System.Web.UI;
using iPortal.Common.Html.Parser;
...
namespace Sys.Common
{
    ...
    public class Utils
    {
        ...
        /// <returns></returns>
        public static string RemoveHtmlContent(string content)
        {
            string str = content;
        ...
        {
            p_SrcString = p_SrcString.Replace('—', ' ');
        ...
        /// <param name="hours"></param>
        /// <returns></returns>
        public static int StrDateDiffDays(string time, int days)
        {
            if (Utils.StrIsNullOrEmpty(time))
                return 1;

            TimeSpan ts = DateTime.Now - DateTime.Parse(time).AddDays(days);
            if (ts.TotalHours > int.MaxValue)
                return int.MaxValue;
            else if (ts.TotalHours < int.MinValue)
                return int.MinValue;
        ...
        /// 创建文件目录
        /// </summary>
        /// <param name="picPath">模块路径</param>
        /// <returns></returns>
        public static string CreateSaveFilePath(string picPath, PathFormat pf)
        {
            try
            {
                //目录拼接
                if (picPath.Substring(picPath.Length - 1, 1) != "/")
                    picPath = picPath + "/";
                string dir = picPath + GetPathFormat(pf) + "/";
                string serverDir = HttpContext.Current.Server.MapPath(dir);
        ...
        /// <param name="picPath">模块路径</param>
        /// <returns></returns>
        public static string CreatePhysicalPath(string picPath, PathFormat pf)
        {
            try
            {
                //目录拼接
        ...
        /// 获取路径格式枚举值
        /// </summary>
        /// <param name="pf">路径格式枚举</param>
        /// <returns></returns>
        static string GetPathFormat(PathFormat pf)
        {
            string strPath = "";
            if (pf == PathFormat.Year_Month)
                strPath = DateTime.Now.Year.ToString() + "-" + DateTime.Now.Month.ToString();
            else if (pf == PathFormat.Year_Month_Day)
                strPath = GetDate();
            else if (pf == PathFormat.YearMonth)
                strPath = DateTime.Now.Year.ToString() + DateTime.Now.Month.ToString();
            else if (pf == PathFormat.YearMonthDay)
                strPath = DateTime.Now.Year.ToString() + DateTime.Now.Month.ToString() + DateTime.Now.Day.ToString();
            return strPath;
        }
        ...
        /// 是否为日期+时间型字符串
        /// </summary>
        /// <param>日期字符串(2008-05-08 15:00:00)</param>
        /// <returns></returns>
        ...
        public static DateTime StrToDatetime(object expression, DateTime dt)
        {
            string strExp = "";
            if (expression != null)
            {
                strExp = expression.ToString().Replace('/','-');
                if (strExp != "" & (IsDateTime(strExp) | IsDateString(strExp)))
                    return Convert.ToDateTime(strExp);
            }
            return dt;
        }

        public static string ObjToString(object expression)
        {
            return expression != null ? expression.ToString() : "";
        }

        /// <summary>
        /// 字符串转换为 decimal,转换失败则为默认 0.00
        /// </summary>
        /// <param name="inputData"></param>
        /// <returns></returns>
        public static decimal StrToDecimal(string inputData)
        {
            if (PageValidate.IsDecimal(inputData))
                return Convert.ToDecimal(inputData);
            return Convert.ToDecimal("0.00");
        }

        #region  工具类整合  王星海 2011-09-09
        ...
        {
            if (GetLoginAdminId() < 1)
            {
                var js = "alert('登陆已超时，请重新登录。');window.top.location.href = 'Login.aspx';";
        ...
        /// 修改时间：2009-06-25
        /// </remarks>
        public static int ConvertChineseNumToInt(string cnum)
        {
            switch (cnum)
            {
                case "一":
        ...
        {
            var list = new List<T>();
            foreach (var t in values.Where(t => list.IndexOf(t) == -1))
            {
                list.Add(t);
            }
            return list.ToArray();
        ...
        public static bool IsNumberArray(string[] strNumber)
        {
            ...
            return strNumber.Length >= 1 && strNumber.All(IsNumber);
        }
        ...
        /// double型转换为int型
        /// </summary>
        /// <param name="dblValue">要转换的double</param>
        /// <param name="valType">0下取整，1上取整</param>
        /// <returns>转换后的int类型结果</returns>
        public static int DoubleToInt(double dblValue, int valType)
        {
            return int.Parse(valType == 1 ? Math.Floor(dblValue).ToString() : Math.Ceiling(dblValue).ToString());
        }
        ...
        {
            if ((strValue == null) || (strValue.ToString() == string.Empty) || (strValue.ToString().Length > 19))
            {
                return defValue;
            }
            ...
            if (val.Length == 19 && IsNumber(firstletter) && long.Parse(firstletter) > 1)
            {
                return defValue;
            }
            if (val.Length == 19 && !IsNumber(firstletter))
            {
                return defValue;
            }


            var longValue = defValue;
            {
                var isLong = new Regex(@"^([-]|[0-9])[0-9]*$").IsMatch(strValue.ToString());
                if (isLong)
                {
                    longValue = Convert.ToInt64(strValue);
                }
            }
        ...
        /// <param name="defValue">缺省值</param>
        /// <returns>转换后的int类型结果</returns>
        public static float StrToFloats(object strValue, float defValue)
        {
            ...
            {
                var isFloat = new Regex(@"^([-]|[0-9])[0-9]*(\.\w*)?$").IsMatch(strValue.ToString());
                if (isFloat)
                {
        ...
        {
            var bantext = ConfigHelper.GetConfigString("FilterSTR");//数据库配置
            return FilterStr(str, bantext);
        }
        /// <summary>
        /// 进行指定的替换(脏字过滤)，替换成***
        /// </summary>
        ...
        {
            string bantext = ConfigHelper.GetConfigString("FilterSTR");//数据库配置
            return FilterStr2(str, bantext);
        }
        ...
        /// </summary>
        /// <param name="str"></param>
        /// <param name="bantext">脏字设置，例如阴户=**,强奸=**</param>
        /// <returns></returns>
        public static string FilterStr2(string str, string bantext)
        {
            var text1 = "";
            var text2 = "";
            var textArray1 = SplitString(bantext, ",");
            foreach (var t in textArray1)
            {
                text1 = t.Substring(0, t.IndexOf("="));
                text2 = text1.Aggregate(text2, (current, t1) => current + "*");
                str = str.Replace(text1, text2);
            }
            return str;
        }
        ...
        public static string HtmlEncodeFull(string strVal)
        {
            if (!string.IsNullOrEmpty(strVal))
            {
                ...
                strVal = strVal.Replace("<", "&lt;");
                strVal = strVal.Replace("\"", "&quot;");
                strVal = strVal.Replace("\r", string.Empty);
                strVal = strVal.Replace("\n", "<br />");
            }
            return strVal;
        }
        ...
        public static string FilterXss(string str)
        {
            ...
            /*if (str.Length != 0)
            {
                string pattern = @"(?:expression|xss:|function|meta|window\.|script|js:|about:|file:|Document\.|vbs:|frame|frameset|object|cookie|on(finish|mouse|Exit=|error|click|key|load|focus|Blur|link|layer|style|base|html|xhtml|xml))";
                pattern = "<(?:(?:.[^>]*" + pattern + "[^>]*)|" + pattern + ")>";
                str = Regex.Replace(str, pattern, "", RegexOptions.IgnoreCase);
            }
            return str;*/
            //使用HtmlParer
            var stcTag = ConfigHelper.GetConfigString("FilterTAG");//数据库配置
            var stcAtt = ConfigHelper.GetConfigString("FilterATT");//数据库配置
            return FilterXss(str, stcTag, stcAtt);
        }
        public static string FilterXss(string strVal, string stcTag, string stcAtt)
        {
            if (!string.IsNullOrEmpty(strVal))
            {
                var packed = new HTMLPacked
                                 {
                                     AllowedTags = stcTag.Split(new char[] { ',' }),
                                     AllowedAttributes = stcAtt.Split(new char[] { ',' })
                                 };
                strVal = packed.Parser(strVal);
            }
            //去除换行
        ...
        /// </summary>
        /// <param name="str"></param>
        /// <param name="bantext">脏字设置，例如阴户=YH,强奸=QJ</param>
        /// <returns></returns>
        public static string FilterStr(string str, string bantext)
        {
            var text1 = "";
            var text2 = "";
            var textArray1 = SplitString(bantext, ",");
        ...
        /// <param name="type">类型：1普通字符串，2Like字符串，3Contains字符串</param>
        /// <returns>判断结果</returns>
        public static string SqlStringFormat(string str, int type)
        {
            ...
            {
                case 1:
                    str = ReplaceSqlKey(str.Replace("'", "''"));
                    break;
                case 2:
                    str = ReplaceSqlKey(str.Replace("[", "[[]"));
                    str = ReplaceSqlKey(str.Replace("%", "[%]"));
                    str = ReplaceSqlKey(str.Replace("_", "[_]"));
                    str = ReplaceSqlKey(str.Replace("'", "''"));
                    break;
                case 3:
                    str = ReplaceSqlKey(str.Replace("[", "[[]"));
        ...
        /// <summary>
        /// 替换输入的字符串中的SQL关键字，防止SQL注入
        /// </summary>
        /// <param name="strRequest">格式化字符串</param>
        ...
        /// <returns>生成随机码</returns>
        public static string MakeRandomString(int pwdlen)
        {
            return MakeRandomString("abcdefghijklmnopqrstuvwxyz0123456789", pwdlen);
        }
        ...
        /// <param name="pwdlen">长度</param>
        /// <returns>生成随机码</returns>
        public static string MakeRandomInt(int pwdlen)
        {
            return MakeRandomString("0123456789", pwdlen);
        ...
        /// <returns></returns>
        public static string GetMetaInfo(string title, string keywords, string description)
        {
            var strBuilder = new StringBuilder();
            strBuilder.Append("<meta name=\"keywords\" content=\"" + keywords + "\" />\r\n");
            strBuilder.Append("<meta name=\"description\" content=\"" + description + "\" />\r\n");
            strBuilder.Append(" <meta name=\"author\" content=\"苏州企业在线\" />");
            strBuilder.Append("<meta name=\"Copyright\" content=\"www.71096.com,苏州企业在线\" />");
        ...
        /// 检查图片是否存在
        /// </summary>
        /// <param name="pathpic">图片路径</param>
        /// <returns>如没有，返回默认图片</returns>
        public static string CheckPic(string pathpic)
        {
            string tempath;
            if (pathpic.IndexOf("productfile") >= 0 || pathpic.IndexOf("productFile") >= 0)
            {
                tempath = @"F:/71096";
            }
            else if (pathpic.IndexOf("productfiles") >= 0 || pathpic.IndexOf("productFiles") >= 0)
            {
                tempath = @"F:/71096";
            }
            else if (pathpic.IndexOf("UploadFiles") >= 0)
            {
                tempath = @"F:/71096WebNet";
            }
            else
            {
                tempath = @"G:/71096web/QyzxWeb";
            }


            var str = FileExists(tempath + pathpic) ? pathpic : "/Users/images/Prdouctdefault.gif";
            return str;
        }
        ...
        /// <returns></returns>
        public static string GetPicWh(string imgPath, int value)
        {
        ...
        /// 返回验证的QQ
        /// </summary>
        /// <param name="mail">传入邮箱地址</param>
        /// <returns>联系QQ</returns>
        public static string ReturnQq(string mail)
        {

            var msg = "";

            var regex = new Regex("^[1-9][0-9]{4,10}@qq.com");
            if (mail == "" || regex.IsMatch(mail.Trim()) == false)
            {
                var rnd = new Random();
                var str = new string[] { "359763443", "1273657129", "1905650613", "2569898766", "2679870395", "2313206289", "2683441965", "2371461289", "2352492590", "1450119509", "1742364648", "2313785837", "1976141457" };
                for (var i = 0; i < str.Length; i++)
                {
                    msg = str[rnd.Next(0, str.Length)];
                    i = str.Length;
                }
            }
            if (regex.IsMatch(mail))
            {
                msg = mail.Substring(0, mail.IndexOf("@"));
            }
            return msg;
        }
        ...
        /// </summary>
        /// <param name="str">字符串</param>
        /// <param name="cod">编码</param>
        /// <returns>解码结果</returns>
        public static string UrlDecode(string str, string cod)
        {
            return HttpUtility.UrlDecode(str, System.Text.Encoding.GetEncoding(cod));
        }
        ...
        /// <param name="getConfigString"></param>
        public static void WriteCookie(string strName, string strValue, int expires, string getConfigString)
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies[strName];
            if (cookie == null)
            {
        ...
        {
            var tempip = "";
            const string strUrl = "http://www.ip138.com/ip2city.asp";
            try
            {
        ...
        public static string Strquotes(string str)
        {
            ...
            {
                if (str.EndsWith(","))
                    str = str.Substring(0, str.Length - 1);//去掉尾部","号
            }
        ...
        public static string GetMd5Str(string str)
        {
            ...
            return str.Replace("1092050956cbb61dadc4917462ab7a9a", "221.224.20.126").Replace("41e3a7818ccd8d9eb56d28268aede418", "SZ_Jgxt").Replace("9aaa8ad297c383230b10ab073f6fb22d", "szjgxt").Replace("1c598d0fee54fad63555b12367549fbb", "szjgxt!@#");
